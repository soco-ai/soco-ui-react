...
// Content under "Knowledge Graph" Tab
class KG extends React.Component {
    ...
    handleDownload = () => {
        if (this.props.links.length !== 0 && Object.entries(this.props.nodes).length !== 0)
            this.props.handleDownload({
                links: this.props.links,
                nodes: this.props.nodes
            }, "knowledge_graph_" + moment().valueOf().toString())
        else
            message.warning("No knowledge graph")
    }
    render() {
        const {
            show_code
        ...
        return (
            !this.props.loading && <div style={{
                ...
                />}
                {
                    (this.props.links.length === 0 || Object.entries(this.props.nodes).length === 0) && 
                    <div style={{fontSize: "18px", width: "100%", textAlign: "center", fontStyle: "italic", color: "#979797", margin: "10px 0 20px"}}>Ready for Search</div>
                }
                {/* Empty div for Drawing the Knowledge Graph when Feeding in Links and Nodes */}
                {
                    <div style={{
                        width: "100%",
                        margin: "0 auto 20px",
                        ...
                        borderRadius: "20px", 
                        position: "relative"}} id={"report-knowledge-graph"}
                    >
